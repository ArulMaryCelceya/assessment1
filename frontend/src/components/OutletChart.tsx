...
export const OutletChart: React.FC<OutletChartProps> = ({ data, loading = false }) => {
  if (loading) {
    return (
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      const item = payload[0].payload as OutletPerformance;
      return (
        <div className="bg-slate-900 border border-slate-700 p-3 rounded-lg shadow-xl text-xs space-y-1">
          <p className="font-bold text-slate-200">{item.outlet_name}</p>
          ...
          <div className="flex justify-between gap-4 text-slate-400 font-mono">
            ...
            <span>{item.items_sold.toLocaleString()}</span>
          </div>
        </div>
      );
    }
  ...
  return (
    <div className="glass-card p-5 h-80 flex flex-col">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-slate-400 font-medium">Ranked Performance</span>
      </div>
